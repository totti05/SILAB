...
class PDF_IG_020 extendS FPDF{ 
   ...
   function Footer(){
            ...
            //Firmas 1er Cuadro
                ...
                $this->text(18,259,$analista->nombre.' '.$analista->apellido.' - '.$analista->ficha);
                    ...
                    if (isset($reportazo->analista_1)){
                      $this->text(18,259,$analista_uno->nombre.' '.$analista_uno->apellido.' - '.$analista_uno->ficha);  
                    }
                }
                //Firma 2
                if($reportazo->analista_2){
                $this->text(50,259,$analista_dos->nombre.' '.$analista_dos->apellido.' - '.$analista_dos->ficha);
                }
                //Firma 3
                if($reportazo->analista_3){
                $this->text(18,269,$analista_tres->nombre.' '.$analista_tres->apellido.' - '.$analista_tres->ficha);
                }
                //Firma 4
                if($reportazo->analista_4){
                $this->text(50,269,$analista_cuatro->nombre.' '.$analista_cuatro->apellido.' - '.$analista_cuatro->ficha);
                }
            ...
            //Firmas 2do Cuadro
                ...
                $this->text(90,265,$jefe_dpto->nombre.' '.$jefe_dpto->apellido.' - '.$jefe_dpto->ficha);
                    ...
                    if(isset($reportazo->jefe_dpto_1)){
                       $this->text(90,265,$jefe_dpto_uno->nombre.' '.$jefe_dpto_uno->apellido.' - '.$jefe_dpto_uno->ficha); 
                    }
                }
                //Firma 2
                if(isset($reportazo->jefe_dpto_2)){
                $this->text(120,265,$jefe_dpto_dos->nombre.' '.$jefe_dpto_dos->apellido.' - '.$jefe_dpto_dos->ficha);  
                }     
            ...
            //codigo de formulario
                $this->SetFont('Arial','',7);
                $this->text(10,287,"holis"."IG-020(17-08-2015)"." H");     
  ...
  //Instanciacion del objeto e impresion del PDF
    ...
    $pdf->AliasNbPages();
    $pdf->AddPage();
    $filePath = 'E:\web_planta\SCP\modules\SILAB\PRUEBA\Temp\ ';
    $pdf->Output(trim($filePath).'doc.pdf',"F");
    $pdf->Output();
    }else{
        unlink('E:\web_planta\SCP\modules\SILAB\PRUEBA\Temp\doc.pdf');
        echo "Negativo Procedimiento";
    }
