...
//////////////SELECT DATOS POR TURNO REDUCCION
	...
	if (pg_num_rows($query_silo_1) > 0)
	{
		...
		if ($n < 5)
		{																											///	AGREGADO 19/11/2017 PARA QUE SE VEA POR PANTALLA EL NIVEL DE SILO PRIMARIO EN TONELADAS
			$reduccion_turno[17][$n]				=	round(($res_silo_1['silo_prim_alumina'] * 100 / 2050), 1);  /// SE CAMBIA 2050 MIGUEL MANEIRO 31/05/2017 B.MENDOZA, D.RAMOS, J.DIAZ  //SE CAMBIO 2000 A 2200 MIGUEL MANEIRO 10/10/2016 SOLICITUD SR. CASTRILLO   
			$reduccion_turno[31][$n]				=	$res_silo_1['silo_prim_alumina'] ;							///	AGREGADO 19/11/2017 PARA QUE SE VEA POR PANTALLA EL NIVEL DE SILO PRIMARIO EN TONELADAS
		}																											///	AGREGADO 19/11/2017 PARA QUE SE VEA POR PANTALLA EL NIVEL DE SILO PRIMARIO EN TONELADAS
		else
		{																											///	AGREGADO 19/11/2017 PARA QUE SE VEA POR PANTALLA EL NIVEL DE SILO PRIMARIO EN TONELADAS
			$reduccion_turno[17][$n]				=	$res_silo_1['silo_prim_alumina'];
			$reduccion_turno[31][$n]				=	($res_silo_1['silo_prim_alumina'] * 2400 / 100);				///	AGREGADO 19/11/2017 PARA QUE SE VEA POR PANTALLA EL NIVEL DE SILO PRIMARIO EN TONELADAS
		}																											///	AGREGADO 19/11/2017 PARA QUE SE VEA POR PANTALLA EL NIVEL DE SILO PRIMARIO EN TONELADAS
		$aux_silo[$n]								=	$res_silo_1['silo_prim_alumina'];							/// 26/05/2017 MIGUEL MANEIRO VARIABLE AUXILIAR CALCULO AUTONOMIA OPERTIVA PLANTA PCO BERNARDO M.
	}
	else
...
//			echo "<br>consumo_promedio: $consumo_promedio[$n]";
			...
			{
				if ((($reduccion_turno[17][$n] * 2050 / 100) - 1302) > 0)
				{
					$reduccion_turno[20][$n]			=	(($reduccion_turno[17][$n] * 2050 / 100) - 1302) / $consumo_promedio[$n];			/// SE CAMBIA 2050 MIGUEL MANEIRO 31/05/2017 B.MENDOZA, D.RAMOS, J.DIAZ 
					$inventario_operativo[$n]			=	(($reduccion_turno[17][$n] * 2050 / 100) - 1302);									/// SE CAMBIA 2050 MIGUEL MANEIRO 31/05/2017 B.MENDOZA, D.RAMOS, J.DIAZ 
					$reduccion_turno[21][$n]			=	round((($reduccion_turno[17][$n] * 2050 / 100) - 1302) / $consumo_promedio[$n], 1);	/// SE CAMBIA 2050 MIGUEL MANEIRO 31/05/2017 B.MENDOZA, D.RAMOS, J.DIAZ 
				}
				else
				{
...
//		echo "<br>consumo_promedio: $consumo_promedio[5]";
		...
		$reduccion_turno[31][7]						=	$tm_torre_5_turno;																	///	AGREGADO 19/11/2017 PARA QUE SE VEA POR PANTALLA EL NIVEL DE SILO PRIMARIO EN TONELADAS
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////
	////////////////////////CALCULO DEUDA ALUMINA SUMINISTROS A REDUCCION//////////////////////////////////////////////////
	//////////////////////MIGUEL MANEIRO 25/05/2017 INCLUIR REQ. BERNY////////////////////////////////////////////////////////

if ($aux_silo[1] + $aux_silo[2] + $aux_silo[3] + $aux_silo[4] < 8200)
{
	$defict_alumina_p19								=	8200 - ($aux_silo[1] + $aux_silo[2] + $aux_silo[3] + $aux_silo[4]);
}
else
{
	$defict_alumina_p19								=	0;
}
if (((((($reduccion_turno[17][5]+$reduccion_turno[17][6])/2)) * 2400/100) + $tm_torre_5_turno) < 3150)
{
	$deficit_alumina_vl								=	2400 + 750 - ((((($reduccion_turno[17][5]+$reduccion_turno[17][6])/2)) * 2400/100) + $tm_torre_5_turno);
}
else
{
	$deficit_alumina_vl								=	0;
}

$auto_oper_suministros[21]						=	$defict_alumina_p19;
$auto_oper_suministros[22]						=	$deficit_alumina_vl;
...
if (pg_num_rows($query_m26) > 0)
{
	...
	if ($res_m26['tm_alumina_fresca'] > 0)				$materias_primas_1		=	$res_m26['tm_alumina_fresca'] -5120 ;				else	$materias_primas[1]		=	0;
...
/*	echo "<br>acumulado_inventario_operativo: $acum_inventario_operativo";
	...
	if($divisor[1]==0) {
		$auto_oper_suministros[12] = "***";
	}
	else {
		$auto_oper_suministros[12]					=	round(($materias_primas[1] - $tm_muertas_autom_silo_princ - $defict_alumina_p19 - $deficit_alumina_vl)/$divisor[1], 1);
	}
