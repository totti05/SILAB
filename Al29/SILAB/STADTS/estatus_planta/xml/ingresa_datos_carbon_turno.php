				...
				<td>
					<div id="tab_datos_igpp" style="width:660px; height:300px;" onMouseOver="limpiar_div(2)"> 
						  <div id="tab1" width="150" name="Datos Originales" align="center" style="font-family:Verdana, Arial, Helvetica, sans-serif ">
									...
									<?=llenar_informacion_carbon_turno($dia, $turno, $db)?><br/>
						  </div>
				</div>
				</td>
			</tr>
...
<?php
	...
	function llenar_informacion_carbon_turno($dia, $turno, $db)
	{
		$sq_m1			=	"SELECT * FROM estatus_planta_carbon_turno WHERE fecha = '$dia' AND turno = '$turno';";
		...
		?>
				...
				<div style="width:100% ">
					<table style="width:80%" border="0" cellpadding="0" cellspacing="0">
						...
						<tr>
							<td class="cuerpo_formulario_ingreso_1"><input type="text" id="anodo_verde_turno" size="4" maxlength="3" value="<?=$res_m1['produc_anodo_verde']?>" onKeyPress="javascript:if(event.keyCode<46 || event.keyCode>57){return false;}"></td>
							<td class="cuerpo_formulario_ingreso_1"><input type="text" id="anodo_cocido_turno" size="4" maxlength="3" value="<?=$res_m1['produc_anodo_cocido']?>" onKeyPress="javascript:if(event.keyCode<46 || event.keyCode>57){return false;}"></td>
							<td class="cuerpo_formulario_ingreso_1"><input type="text" id="anodo_envarillado_turno" size="4" maxlength="3" value="<?=$res_m1['produc_anodo_envarillado']?>" onKeyPress="javascript:if(event.keyCode<46 || event.keyCode>57){return false;}"></td>
						</tr>
					</table>
					<br/>
					<input type="button" value="Procesar Turno" onClick="guarda_datos_carbon_turno()">
				</div>
		<?php
	}
?>