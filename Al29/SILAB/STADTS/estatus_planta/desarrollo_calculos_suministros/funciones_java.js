...
function boton_actualiza_auxiliares(cual_dep)	/////probado
{
	var aleatorio 			= 	Math.random();
	poststr					=	'dia='+document.getElementById('fecha_carga_auxiliares').value+
								'&turno='+document.getElementById('select_turno').value+
								'&aleatorio='+aleatorio;
	if (document.getElementById('oculto_modulo')) 	modulo	=	document.getElementById('oculto_modulo').value;
	else											modulo	=	"estatus_planta";
	var ob_ajax1 			=	null;
	if (cual_dep == 1)
		makePOSTRequest(1, 0, 'modules/'+modulo+'/xml/ingresa_datos_niv_silo_turno_fase_densa.php?'+poststr, poststr, ob_ajax1, 'tabla_datos_auxiliares');
	if (cual_dep == 2)
		makePOSTRequest(1, 0, 'modules/'+modulo+'/xml/ingresa_datos_niv_silo_turno_manej_mat.php?'+poststr, poststr, ob_ajax1, 'tabla_datos_auxiliares');
}
...
function limpiar_div(cual_div)	/////probado
{
	...
	if (cual_div == 22)
	{
		document.getElementById('respuesta_ingresa_datos_auxiliares').innerHTML				=	"";
	}
	if (cual_div == 20)
	{
...
{
	var aleatorio =  Math.random();
	var	dpto_observ = document.getElementById('dpto_observ_usuario').value;
	if (document.getElementById('oculto_modulo')) 	modulo	=	document.getElementById('oculto_modulo').value;
	else											modulo	=	"estatus_planta";
	var poststr				=	'dia='+document.getElementById('cuadro_fecha_carga_datos_turno').value+
							'&turno='+document.getElementById('select_turno').value+
							'&dpto_observ='+dpto_observ+
							'&aleatorio='+aleatorio;
	var ob_ajax1 		=	null;
	...
	if (area == 6) makePOSTRequest(1, 0, 'modules/'+modulo+'/xml/ingresa_datos_observacion_mtto.php?'+poststr, poststr, ob_ajax1, 'tabla_datos_observacion');
}
...
function guarda_datos_observaciones(sub_area, area)
{
	...
	var usuario							=	document.getElementById('usuario_modulo').value;
	
	 var limitField	=	document.getElementById('observacion_sub_area_'+sub_area);
	limitNum	=	187;
	
	if (limitField.value.length > limitNum) 
	{
		limitField.value = limitField.value.substring(0, limitNum);
		alert('Max. 186 caracteres para la observacion');
		return
	}
...
function ventana_detalles_2(tipo_variable, tipo_consulta, dia, mes, ano, turno_reporte)
{
		...
		window.open('desarrollo_tabla_detalles_equipo_movil_2.php?'+poststr, "REPORTE_IGPP2");
}
...
function ventana_detalles_6(tipo_variable, tipo_consulta, dia, mes, ano, linea, turno_reporte)
{
		var aleatorio =  Math.random();
		var poststr 						=	"tipo_variable="+tipo_variable+
											"&linea="+linea+
											...
											"&fecha_reporte="+dia+'/'+mes+'/'+ano+
											"&aleatorio="+aleatorio;
		window.open('tabla_detalles_celdas_2.php?'+poststr, "_blank","status=0, toolbar=0, width=360, height=450, scrollbars=1");
}
/////////////////////////////////
//////////////////////////////////////
...
function guarda_datos_nivel_silos_turno(cual_dep)	/////probado
{
	var aleatorio =  Math.random();
	var arreglo_nivel_silo_pirm_alumina_linea			=	new Array();
	var arreglo_nivel_silo_sec_alumina_linea			=	new Array();
	var arreglo_nivel_silo_bano_linea					=	new Array();
	var fecha;
	var turno;

	fecha												=	document.getElementById('fecha_carga_auxiliares').value;
	turno												=	document.getElementById('select_turno').value;
	var	n_filas = 0;
	var	n_filas2 = 1;	
	if (cual_dep == 1)
	{
		while(n_filas2 <= 5)
		{
			arreglo_nivel_silo_pirm_alumina_linea[n_filas]	=	document.getElementById('silo_prim_alum_'+n_filas2).value;
			n_filas++;
			n_filas2++;	
		}
		poststr							=	"fecha="+fecha+
											"&turno="+turno+
											"&control="+cual_dep+
											"&arreglo_nivel_silo_pirm_alumina_linea="+arreglo_nivel_silo_pirm_alumina_linea+
				  							"&aleatorio="+aleatorio;
		...
		makePOSTRequest(1, 0, 'modules/'+modulo+'/xml/procesa_ingresa_datos_auxiliares_turno.php?'+poststr, poststr, ob_ajax1, 'repuesta_servidor_auxiliares');
		//document.getElementById('myspan').innerHTML = 'modules/'+modulo+'/xml/procesa_ingresa_datos_auxiliares_turno.php?'+poststr;
	}
	else
	{
		while(n_filas2 < 7)
		{
			arreglo_nivel_silo_sec_alumina_linea[n_filas]	=	document.getElementById('silo_sec_alum_'+n_filas2).value;
			n_filas++;
			n_filas2++;	
		}
		...
		while(n_filas2 < 6)
		{
			arreglo_nivel_silo_bano_linea[n_filas]			=	document.getElementById('silo_prim_bano_'+n_filas2).value;
			n_filas++;
			n_filas2++;	
		}
		
		///// ARREGLO SILO PRIMARIO DE ALUMINA DE LINEA 5
		arreglo_nivel_silo_pirm_alumina_linea[0]	=	document.getElementById('silo_prim_alum_5').value;   ///SILO SALA 9
		arreglo_nivel_silo_pirm_alumina_linea[1]	=	document.getElementById('silo_prim_alum_6').value;   ///SILO SALA 10
		
		
		poststr							=	"fecha="+fecha+
											"&turno="+turno+
											"&control="+cual_dep+
											"&arreglo_nivel_silo_sec_alumina_linea="+arreglo_nivel_silo_sec_alumina_linea+
											"&arreglo_nivel_silo_bano_linea="+arreglo_nivel_silo_bano_linea+
											"&arreglo_nivel_silo_pirm_alumina_linea="+arreglo_nivel_silo_pirm_alumina_linea+
				  							"&aleatorio="+aleatorio;
...
function boton_actualiza_plan_pco_suministros()	/////probado
{
	var aleatorio =  Math.random();
	poststr					=	'mes_busc='+document.getElementById('select_mes_ajuste').value+
								'&anno_busc='+document.getElementById('select_año_ajuste').value+
								'&aleatorio='+aleatorio;
	if (document.getElementById('oculto_modulo')) 	modulo	=	document.getElementById('oculto_modulo').value;
	else											modulo	=	"estatus_planta";
	var ob_ajax1 			=	null;
	document.getElementById('contenedor_tabla_select_suministros').style.visibility = "hidden";
	makePOSTRequest(1, 0, 'modules/'+modulo+'/xml/ingresa_datos_planes_pco_materias_primas.php?'+poststr, poststr, ob_ajax1, 'tabla_datos_plan_pco_suministros');
}

function procesa_ingresa_datos_plan_pco_materias_primas()
{
	var aleatorio =  Math.random();
	poststr					=	'oculto_mes_busc='+document.getElementById('oculto_mes_busc').value+
								'&oculto_anno_busc='+document.getElementById('oculto_anno_busc').value+
								'&inventarios_1='+document.getElementById('inventarios_1').value+
								'&inventarios_2='+document.getElementById('inventarios_2').value+
								'&inventarios_3='+document.getElementById('inventarios_3').value+
								'&inventarios_4='+document.getElementById('inventarios_4').value+
								'&inventarios_5='+document.getElementById('inventarios_5').value+
								'&inventarios_6='+document.getElementById('inventarios_6').value+
								'&inventarios_7='+document.getElementById('inventarios_7').value+
								'&aleatorio='+aleatorio;
	if (document.getElementById('oculto_modulo')) 	modulo	=	document.getElementById('oculto_modulo').value;
	else											modulo	=	"estatus_planta";
	var ob_ajax1 			=	null;
	document.getElementById('contenedor_tabla_select_suministros').style.visibility = "visible";
	makePOSTRequest(1, 0, 'modules/'+modulo+'/xml/procesa_ingresa_datos_planes_pco_materias_primas.php?'+poststr, poststr, ob_ajax1, 'ingresa_db_datos_plan_pco_suministros');
}